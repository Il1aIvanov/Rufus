        ...
        if (Input.touchCount > 0 && !locked)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if (Mathf.Abs(transform.position.x - formPlace1.position.x) <= 0.5f &&
                        Mathf.Abs(transform.position.y - formPlace1.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(formPlace1.position.x, formPlace1.position.y);
                        locked = true;
                    }
                    else
                    {
